...
def main(): 
    ...
    images_file_list = os.listdir(args.dir)

    #run opencv_createsamples for each image in directory
    for image_name in images_file_list:
        image_file = os.path.join(args.dir,image_name)
        vec_file = args.vec + image_name + '.vec'
    ...
    '''
    find all files that match <args.vec + image_name>, create directory, move all files to that directory

    shutil.move???
    
    vec_path = os.path.dirname(os.path.abspath(
    for vec_file_list
        os.rename()
